    ...
    <div class="bg-body-light">
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                <h1 class="flex-sm-fill font-size-h2 font-w400 mt-2 mb-0 mb-sm-2">Nationalities</h1>
            ...
            <div class="block-header block-header-default">
                ...
                <div class="btn-group btn-group-sm pull-right" role="group">
                    <div class="block-options pl-3 pr-2">
                        <a href="{{ route('nationalities.create') }}" class="btn-block-option" title="Create New Country">
    ...
    <script>
        $(document).ready(function() {
            var nationalities_datatable = $('#nationalities_datatable').DataTable( {
                ...
                searching: true,
                ajax: {
                    url: "{{ route('api.nationalities.search') }}",
                    type: "POST",
                    headers: {
                ...
                columns: [
                    { data: 'id', className: "text-center", width: "80px",
                        render: function(data, type, full, meta) {
                            return '<a href="{{ route("nationalities.index")}}/' + data + '">' + data + '</a>';
                        }
                    },
                    { data: 'code', className: "font-w600" },
                    { data: 'name_de', className: "d-none d-sm-table-cell", width: "30%" },
                    { data: 'name_en', className: "d-none d-sm-table-cell", width: "15%" },
                    { data: 'id', className: "text-right", width: "15%", orderable: false,
                        render: function(data, type, full, meta) {
                            return '<a href="{{ route("nationalities.index")}}/' + data + '/edit" class="btn btn-sm btn-info mr-1" title="Edit Nationality"><i class="si si-pencil"></i></a>' +
                                '<a href="{{ route("nationalities.index")}}/' + data + '" class="btn btn-sm btn-primary" title="Show Nationality"><i class="si si-doc"></i></a>';
                        }
                    },
                ],
            });
