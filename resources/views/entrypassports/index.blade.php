    ...
    <div class="bg-body-light">
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                <h1 class="flex-sm-fill font-size-h2 font-w400 mt-2 mb-0 mb-sm-2">Entry Passports</h1>
                <nav class="flex-sm-00-auto ml-sm-3" aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">Settings</li>
                        <li class="breadcrumb-item">Basicdatas</li>
                        <li class="breadcrumb-item active" aria-current="page">Entry Passport</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <div class="content content-full">
        ...
        <div class="block block-rounded block-bordered dynamic_container">
            <div class="block-header block-header-default">
                ...
                <div class="btn-group btn-group-sm pull-right" role="group">
                    <div class="block-options pl-3 pr-2">
                        <a href="{{ route('entrypassports.create') }}" class="btn-block-option" title="Create New Entry Passport">
            ...
            <div class="block-content block-content-full">
                ...
                <table id="entrypassports_datatable" class="table table-bordered table-striped table-vcenter">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Content</th>
...
@section('js_after')
    ...
    <script>
        $(document).ready(function() {
            var entrypassports_datatable = $('#entrypassports_datatable').DataTable( {
                serverSide: true,
                processing: true,
                searching: true,
                ajax: {
                    url: "{{ route('api.entrypassports.search') }}",
                    type: "POST",
                    headers: {
                ...
                columns: [
                    { data: 'id', className: "text-center", width: "80px",
                        render: function(data, type, full, meta) {
                            return '<a href="{{ route("entrypassports.index")}}/' + data + '">' + data + '</a>';
                        }
                    },
                    ...
                    { data: 'id', className: "text-right", width: "15%", orderable: false,
                        render: function(data, type, full, meta) {
                            return '<a href="{{ route("entrypassports.index")}}/' + data + '/edit" class="btn btn-sm btn-info"title="Edit Entry Passport" style="margin-right:5px;"><i class="si si-pencil"></i></a>' +
                                '<a href="{{ route("entrypassports.index")}}/' + data + '" class="btn btn-sm btn-primary" title="Show Entry Passport"><i class="si si-doc"></i></a>';
                        }
                    },
                ],
            });
