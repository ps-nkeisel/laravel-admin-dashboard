    ...
    <div class="bg-body-light">
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                <h1 class="flex-sm-fill font-size-h2 font-w400 mt-2 mb-0 mb-sm-2">Check Entry passport assign</h1>
            ...
            <div class="block-content">
                <form method="POST" enctype="multipart/form-data" id="form_nats_search">
                    <div class="row">
                        ...
                        <div class="col-6">
                            <h4 class="font-w400">Passports</h4>

                            @foreach($entrypassports as $entrypassport)
                                <div class="form-group">
                                    <div class="custom-control custom-checkbox custom-control-lg custom-control-inline">
                                        <input type="checkbox" class="entrypassport custom-control-input" id="entrypassport-{{ $entrypassport->id }}"
                                            name="entrypassports[]" value="{{ $entrypassport->id }}">
                                        <label class="custom-control-label" for="entrypassport-{{ $entrypassport->id }}">{!! $entrypassport->content_en !!}</label>
                                    </div>
                                </div>
                            @endforeach
                        </div>
    ...
    <script>
        ...
        // Nationalities multi select
            ...
            var nationalities_datatable = $('#nationalities_datatable').DataTable( {
                ...
                searching: false,
                ajax: {
                    url: "{{ route('api.entries.check.passassign') }}",
                    type: "POST",
                    headers: {
                    ...
                    data: function ( d ) {
                        ...
                        formData.forEach(function(element) {
                            ...
                            } else if (element.name == 'entrypassports[]') {
                                d.filters.entrypassport_ids.push(element.value);
                            }
                        })
                    },
                },
                ...
                columns: [
                    ...
                    { data: 'passassign', className: "text-center" },
                ],
                createdRow: function( row, data, dataIndex) {
                    if (data.passassign == 'no') {
            ...
            $('#form_nats_search').submit(function(event) {
                ...
                if ($('.entrypassport:checked').length === 0) {
                    alert('Choose at least one entrypassport');
                    return;
                }
