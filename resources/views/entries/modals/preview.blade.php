<!-- EntryPreview Modal Start -->

    <div class="modal fade" id="modalEntryPreview" tabindex="-1" role="dialog" aria-labelledby="modalEntryPreview">
...
<script>
    $(document).ready(function() {
        $('#modalEntryPreview').on('show.bs.modal', function (e) {
            var modal = $(this);
            modal.find('.block').addClass('block-mode-loading');

            $.ajax({
                type: 'POST',
                url: '{{ route("api.entries.preview") }}',
        ...
        $('.btn-create-pdf').click(function() {
            var formAction = $('.entry-form').attr('action');

            $('.entry-form').attr('action', '{{ route("entries.preview") }}');
            $('.entry-form').attr('target', '_blank');

            $('.entry-form').submit();

            $('.entry-form').attr('action', formAction);
            $('.entry-form').removeAttr('target');
        })
    })
</script>
