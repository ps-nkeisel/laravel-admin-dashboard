    ...
    <div class="bg-body-light">
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                <h1 class="flex-sm-fill font-size-h2 font-w400 mt-2 mb-0 mb-sm-2">Health No Infos available</h1>
                <nav class="flex-sm-00-auto ml-sm-3" aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">Conditions</li>
                        <li class="breadcrumb-item active" aria-current="page">Health no info available</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <script>
        $(document).ready(function() {
            var inoculations_datatable = $('#inoculations_datatable').DataTable( {
                ...
                searching: true,
                ajax: {
                    url: "{{ route('api.inoculations.check.noinfos') }}",
                    type: "POST",
                    headers: {
                ...
                columns: [
                    ...
                    { data: 'id', className: "text-right", width: "15%", orderable: false,
                        render: function(data, type, full, meta) {
                            return '<a href="{{ route("inoculations.index")}}/' + data + '/edit" class="btn btn-sm btn-info mr-1" title="Edit Health"><i class="si si-pencil"></i></a>' +
                                '<a href="{{ route("inoculations.index")}}/' + data + '" class="btn btn-sm btn-primary" title="Show Health"><i class="si si-doc"></i></a>';
                        }
                    },
                ],
            });
