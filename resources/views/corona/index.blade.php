    ...
    <div class="bg-body-light">
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                <h1 class="flex-sm-fill font-size-h2 font-w400 mt-2 mb-0 mb-sm-2">Corona</h1>
                <nav class="flex-sm-00-auto ml-sm-3" aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">Settings</li>
                        <li class="breadcrumb-item active" aria-current="page">Corona</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <div class="content content-full">
        ...
        <div class="block block-rounded block-bordered dynamic_container">
            ...
            <div class="block-content block-content-full">
                ...
                <table id="corona_datatable" class="table table-bordered table-striped table-vcenter">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Country Code</th>
...
@section('js_after')
    ...
    <script>
        $(document).ready(function() {
            var corona_datatable = $('#corona_datatable').DataTable( {
                serverSide: true,
                processing: true,
                searching: true,
                ajax: {
                    url: "{{ route('api.corona.search') }}",
                    type: "POST",
                    headers: {
                ...
                columns: [
                    { data: 'id', className: "text-center", width: "80px",
                        render: function(data, type, full, meta) {
                            return '<a href="{{ route("corona.index")}}/' + data + '">' + data + '</a>';
                        }
                    },
                    { data: 'countrycode', className: "font-w600" },
                    { data: 'updated_at', className: "d-none d-sm-table-cell", width: "30%" },
                    { data: 'updated_user', className: "d-none d-sm-table-cell", width: "15%" },
                    { data: 'id', className: "text-right", width: "15%", orderable: false,
                        render: function(data, type, full, meta) {
                            return '<a href="{{ route("corona.index")}}/' + data + '/edit" class="btn btn-sm btn-info mr-1" title="Edit Corona"><i class="si si-pencil"></i></a>' +
                                '<a href="{{ route("corona.index")}}/' + data + '" class="btn btn-sm btn-primary" title="Show Corona Info"><i class="si si-doc"></i></a>';
                        }
                    },
                ],
            });
