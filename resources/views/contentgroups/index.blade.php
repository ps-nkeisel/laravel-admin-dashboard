    ...
    <div class="bg-body-light">
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                <h1 class="flex-sm-fill font-size-h2 font-w400 mt-2 mb-0 mb-sm-2">Contentgroups</h1>
    ...
    <div class="content content-full">
        ...
        <div class="block block-rounded block-bordered dynamic_container">
            ...
            <div class="block-content block-content-full">
                ...
                <table id="contentgroups_datatable" class="table table-bordered table-striped table-vcenter">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Type</th>
...
@section('js_after')
    ...
    <script>
        $(document).ready(function() {
            var contentgroups_datatable = $('#contentgroups_datatable').DataTable( {
                serverSide: true,
                processing: true,
                searching: true,
                ajax: {
                    url: "{{ route('api.contentgroups.search') }}",
                    type: "POST",
                    headers: {
                ...
                columns: [
                    { data: 'id', className: "text-center", width: "80px",
                        render: function(data, type, full, meta) {
                            return '<a href="{{ route("contentgroups.index")}}/' + data + '">' + data + '</a>';
                        }
                    },
                    { data: 'contentadditionalable_type', className: "d-none d-sm-table-cell", width: "20%" },
                    { data: 'section', className: "d-none d-sm-table-cell", width: "10%" },
                    { data: 'code', className: "d-none d-sm-table-cell", width: "10%" },
                    { data: 'content', className: "d-none d-sm-table-cell", width: "30%" },
                    { data: 'position', className: "d-none d-sm-table-cell", width: "10%" },
                    { data: 'id', className: "text-right", width: "15%", orderable: false,
                        render: function(data, type, full, meta) {
                            return '<a href="{{ route("contentgroups.index")}}/' + data + '/edit" class="btn btn-sm btn-info"title="Edit Contentgroup" style="margin-right:5px;"><i class="si si-pencil"></i></a>' +
                                '<a href="{{ route("contentgroups.index")}}/' + data + '" class="btn btn-sm btn-primary" title="Show Contentgroup"><i class="si si-doc"></i></a>';
                        }
                    },
                ],
            });
